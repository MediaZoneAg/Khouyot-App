      ...
      children: [
        ...
        BlocBuilder<HomeCubit, HomeState>(
          builder: (context, state) {
            return Skeletonizer(
              enabled: state is ProductAllLoading,
              child: SizedBox(
                width: 350.w,
                height: 280.h,
                child: Stack(
                  children: [
                    PageView.builder(
                      ...
                      itemCount: widget.productModel.data?.first.variants?.first
                              .image?.length ??
                          0,
                      onPageChanged: (index) {
                      ...
                      itemBuilder: (context, index) {
                        return GestureDetector(
                          ...
                          child: Hero(
                            tag: widget.productModel.data?.first.variants?.first
                                    .image?[index] ??
                                '',
                            child: AppCachedNetworkImage(
                                image: widget.productModel.data?.first.variants
                                        ?.first.image?[index] ??
                                    '',
                                width: 350.w,
                                height: 350.h,
            ...
            return SizedBox(
              ...
              child: Skeletonizer(
                ...
                child: ListView.builder(
                  ...
                  itemCount: widget
                      .productModel.data?.first.variants?.first.image?.length,
                  itemBuilder: (context, index) {
                    return GestureDetector(
                      onTap: () {
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          border: Border.all(
                            color: HomeCubit.get(context).currentImageIndex ==
                                    index
                        ...
                        child: AppCachedNetworkImage(
                          width: 68.w,
                          height: 70.h,
                          image: widget.productModel.data?.first.variants?.first
                                  .image?[index] ??
                              '',
                          radius: 8,
                        ),
  ...
  Future<void> favFunction() async {
    if (await CashHelper.getStringSecured(key: Keys.token) != "") {
      if (FavCubit.get(context)
          .favorite
          .contains(widget.productModel.data?.first.id!)) {
        final favModel =
            widget.productModel.data?.first.variants?.first.convertToFav(
          productName: widget.productModel.data?.first.name,
          description: widget.productModel.data?.first.description,
        );
